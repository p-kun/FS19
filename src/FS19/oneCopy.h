...
public:
		...
		while( ::PeekMessage( &msg, NULL, 0, 0, PM_NOREMOVE ) ) {
			if( msg.message == WM_QUIT ) {
				...
				if( msg.wParam == VK_ESCAPE ) {
					if( ::MessageBox( hWnd, _T( "コピー中です。中止しますか？" ), _T( "どうする？" ), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2 ) == IDYES ) {
						res = 1;
						break;
					}
				}
	...
	BOOL CopyExec( dirent *top, const TCHAR *dir ) {
		...
		for( elm = top; elm; elm = elm->next ) {
			//
			if( elm->bookMark == TCOPYBMARK_NULL || elm->bookMark == TCOPYBMARK_NOT_COPY_FOR_DATE || elm->bookMark == TCOPYBMARK_NOT_COPY_FOR_FILE ) {
				continue;
			}
		...
		for( elm = top; elm && !_act.cancel; elm = elm->next ) {
			...
			else if( elm->bookMark == TCOPYBMARK_FAIL_IF_EXISTS || elm->bookMark == TCOPYBMARK_RENAME ) {
